...
Updated.df[Topic == 1][1:10,.(text)]
ggplot(Updated.df[Topic == 1], aes(x = followers_count)) + geom_histogram(binwidth = 10) + xlim(c(0,300))
ggplot(Updated.df[Topic == 1], aes(x = account_created_at)) +
  geom_histogram()
dim(Updated.df[Topic==1])[1]
Updated.df[Topic==1, .(hashtags)] %>% 
  unlist %>% table %>% 
  sort(decreasing = T)
Updated.df[Topic==2][1:10,.(text)]
tweets.excl.Topic1 = Updated.df[Topic !=1]
